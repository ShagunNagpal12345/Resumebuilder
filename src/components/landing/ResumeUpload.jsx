...
const ResumeUpload = ({ onFileUpload }) => { // <--- Receive Prop
  ...
  return (
    <div className="theme-app-shell min-h-screen w-full flex flex-col items-center justify-center relative overflow-hidden font-sans selection:bg-teal-500/30">
      
      {/* BACKGROUND FX */}
      <div className="theme-grid-overlay absolute inset-0 pointer-events-none"></div>
      <div className="absolute top-0 left-0 w-[600px] h-[600px] bg-teal-400/10 blur-[120px] rounded-full pointer-events-none" />
      <div className="absolute bottom-0 right-0 w-[600px] h-[600px] bg-sky-300/15 blur-[120px] rounded-full pointer-events-none" />
      ...
      <div className="relative z-10 w-full max-w-[1400px] px-6 md:px-12 py-12 grid lg:grid-cols-12 gap-12 lg:gap-20 items-center">
        ...
        <div className="lg:col-span-7 flex flex-col justify-center space-y-12">
          <div className="space-y-6">
            ...
            <h1 className="text-5xl md:text-7xl font-black tracking-tight leading-tight text-[color:var(--theme-text-primary)]">
              Your Resume is <br/>
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-teal-400 to-emerald-400">
                The Blueprint.
              </span>
            </h1>
            <p className="max-w-2xl text-lg leading-relaxed text-[color:var(--theme-text-secondary)]">
              Upload your CV. Our AI analyzes your experience against industry standards to generate a professional, optimized resume instantly.
            </p>
          </div>

          <div>
            <h3 className="mb-6 flex items-center gap-2 text-sm font-bold uppercase tracking-widest text-[color:var(--theme-text-muted)]">
              <Users size={16} /> Optimized For
            </h3>
            <div className="grid sm:grid-cols-3 gap-5">
              {interviewers.map((person, i) => (
                <div key={i} className="theme-section-surface group relative rounded-2xl p-4 transition-all duration-300 hover:-translate-y-1 hover:border-teal-300">
                  <div className="flex items-center gap-4 mb-3">
                    <div className="h-12 w-12 shrink-0 overflow-hidden rounded-full border-2 border-[color:var(--theme-border-soft)] transition-colors group-hover:border-teal-400">
                      <img src={person.img} alt={person.name} className="w-full h-full object-cover" />
                    </div>
                    <div className="overflow-hidden">
                      <div className="truncate text-sm font-bold text-[color:var(--theme-text-primary)] transition-colors group-hover:text-teal-600">{person.name}</div>
                      <div className="truncate text-[10px] font-medium uppercase text-[color:var(--theme-text-secondary)]">{person.role}</div>
                    </div>
                  </div>
                  <div className="flex items-center gap-1.5 border-t border-[color:var(--theme-border-soft)] pt-3 text-xs text-[color:var(--theme-text-secondary)]">
                    <Brain size={12} className="text-teal-500" /> Focus: <span className="text-[color:var(--theme-text-primary)]">{person.focus}</span>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="flex flex-wrap gap-8 border-t border-[color:var(--theme-border-soft)] pt-4 text-sm font-medium text-[color:var(--theme-text-secondary)]">
             <div className="flex items-center gap-2"><ShieldCheck size={18} className="text-teal-500" /> Private & Encrypted</div>
             <div className="flex items-center gap-2"><Brain size={18} className="text-teal-500" /> AI Powered Extraction</div>
          </div>
        </div>
        ...
        <div className="lg:col-span-5 flex flex-col justify-center">
          <div className="theme-section-glass rounded-[32px] p-2 relative overflow-hidden group">
            ...
            <div className="theme-section-surface relative rounded-[24px] p-8 md:p-12 h-full flex flex-col">
                <div className="mb-8 text-center md:text-left">
                    <h2 className="mb-2 text-3xl font-bold text-[color:var(--theme-text-primary)]">Upload Resume</h2>
                    <p className="text-sm text-[color:var(--theme-text-secondary)]">Upload PDF or DOCX, then choose exact import or AI-optimized import.</p>
                </div>
                ...
                <div 
                    ...
                    className={`
                        ...
                        ${file 
                            ...
                            : isDragging 
                                ? 'border-teal-400 bg-teal-500/10 scale-[1.02]' 
                                : 'border-[color:var(--theme-border-soft)] bg-[color:var(--theme-surface-subtle)] hover:border-teal-300 hover:bg-white'
                        }
                    `}
                >
                    <input 
                    ...
                    <div className="absolute inset-0 bg-[linear-gradient(45deg,transparent_25%,rgba(69,143,131,0.04)_25%,rgba(69,143,131,0.04)_50%,transparent_50%,transparent_75%,rgba(69,143,131,0.04)_75%,rgba(69,143,131,0.04)_100%)] bg-[size:20px_20px] pointer-events-none"></div>
                    ...
                    {file ? (
                        <div className="relative z-10 animate-in zoom-in duration-300">
                            ...
                            <p className="mx-auto max-w-[240px] truncate text-xl font-bold text-[color:var(--theme-text-primary)]">{file.name}</p>
                            <p className="text-teal-400 text-sm font-bold uppercase mt-2 tracking-wider flex items-center justify-center gap-2">
                                <CheckCircle size={16} /> Ready to Process
                            </p>
                            <span className="mt-6 block text-xs text-[color:var(--theme-text-secondary)] opacity-70 transition-opacity group-hover:opacity-100">Tap to change file</span>
                        </div>
                    ) : (
                        <div className="relative z-10">
                            <div className={`mx-auto mb-8 flex h-20 w-20 items-center justify-center rounded-full transition-all duration-300 ${isDragging ? 'bg-teal-500 text-white scale-110 shadow-lg shadow-teal-500/50' : 'bg-[color:var(--theme-accent-soft)] text-teal-700 shadow-inner'}`}>
                                <Upload className="w-9 h-9" />
                            </div>
                            <p className="mb-2 text-xl font-bold text-[color:var(--theme-text-primary)]">Drag & drop resume</p>
                            <p className="text-sm text-[color:var(--theme-text-secondary)]">or click to browse files</p>
                        </div>
                    )}
                </div>
